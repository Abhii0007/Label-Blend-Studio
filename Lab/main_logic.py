...
class ImageGenerator(QMainWindow):
    ...
    def generate_and_save_segmentations(self):
        ...
        os.makedirs("output_seg/images", exist_ok=True)
        os.makedirs("output_seg/labels", exist_ok=True)
        ...
        def generate(i):
            ...
            result.save(f"output_seg/images/image{image_index}.png")
            with open(f"output_seg/labels/image{image_index}.txt", "w") as f:
                f.write("\n".join(label_lines))
...
#------------------changes made------------------/          
    ...
    def generate_and_save_images(self):
        try:
            count = int(self.num_images_input.text())
            start_index = int(self.starting_image_input.value())  # <-- New input
        except ValueError:
            return
        ...
        image_dir = os.path.join("output_box", "images")
        label_dir = os.path.join("output_box", "labels")
        os.makedirs(image_dir, exist_ok=True)
        os.makedirs(label_dir, exist_ok=True)

        def generate_and_save(i):
            image_index = start_index + i  # <-- Adjusted image index
